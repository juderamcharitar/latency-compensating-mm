...
A limit order is more or less likely to be reached depending on where it is placed. This study asks whether the visible state of the order book adds useful information beyond those placement choices. Using cryptocurrency order-book snapshots, the analysis finds clear additional predictive information for BTC/USD and ETH/USD, but not a reliable effect for SOL/USD in the primary time-ordered test. The results also show that conclusions can change when train and test periods occupy different market regimes, making careful validation and simple benchmarks especially important in market-prediction research.
